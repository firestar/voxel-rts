...
`B` cycles through `ALL_BUILDINGS`: play → Barracks → Farm → Storage → Power
Plant → Refinery → Tech Lab → play. Hotkeys `1`..`6` jump directly to a spec
(and enter build mode if not already in it). `P` toggles plant mode. The
single `Mode = 'play' | 'build' | 'plant'` enum keeps the input dispatch in
`handleRelease` flat — `isBuildMode` and `activeBuildSpec` centralise the
"which spec are we placing?" decision so the ghost preview, footprint check,
and `buildings.place` call all read from the same source (`this.buildSpec`).
